            ...
            >
                <Row style={{     display: 'flex', alignItems: 'center'}}>
                    ...
                    <div className="hero-fourth-text">
                        ...
                        <Button href="https://github.com/chaummixer/chaummixer-wallet-electron/releases/download/v1.0.0/chaummixer-1.0.0.Setup.exe" color="secondary">Download windows setup</Button> {<br/>}
                        <a href="https://github.com/chaummixer/chaummixer-wallet-electron/releases/download/v1.0.0/chaummixer-1.0.0.Setup.exe.asc" style={{color: '#fff'}}><FontAwesomeIcon icon={faPaperclip} />Accompanying signature</a>
                    ...
                    <div className="hero-fourth-text">
                        ...
                        <Button href="https://github.com/chaummixer/chaummixer-wallet-electron/releases/download/v1.0.0/chaummixer_1.0_amd64.deb" color="secondary">Download Linux setup</Button> {<br/>}
                        <a href="https://github.com/chaummixer/chaummixer-wallet-electron/releases/download/v1.0.0/chaummixer_1.0_amd64.deb.asc" style={{color: '#fff'}}><FontAwesomeIcon icon={faPaperclip} />Accompanying signature</a>
                    </Col>
                    {/* <Col xs="12" sm="4" md="4" lg="4">
                    <div className="hero-fourth-text">
                        <p >SHAsum of both .deb and .exe files: </p>
                    </div>
                        <Button href="https://github.com/chaummixer/chaummixer-wallet-electron/releases/download/v0.0.1/chaummixer_0.0.1_SHASUMS256.asc" color="secondary">Download SHASUMS </Button> {<br/>}
                    </Col> */}
                </Row>
            </div>
        </div>
...
const HowandWhy = () => { 
    ...
    <p>
        This is a valid question. An electron implementation of chaummixer has been created for a number of purposes, with the most notable one of them being: heightened security and privacy.
    </p>
    <p>
      When you load the chaummixer wallet in your browser, you assume that <code>chaummixer.com</code> belongs to <i>"us"</i>, the <i>"original"</i> chaummixer developers. However, through malice, error, or other circumstances, it can happen that either the domain name, or the wallet script is corrupted and or harmful.
...
<Col className="fee-box list-group-item-success" sm={{ size: 12, offset: 3 }}> 
    ...
    <p>
        Alternatively, you can calculate the hash of the redistributable yourself, and compare it against our signed hashes, <a href="https://github.com/chaummixer/chaummixer-wallet-electron/releases/download/v1.0.0/shasums.asc">which can be found here</a>.
    </p>
    </SectionDiv>)
}
...
const LatestUrl = () => { 
    ...
    const getData = () =>fetch(`https://updates.chaummixer.dev/`).then((res) => res.status === 200 ? res.json() : null).catch(error => alert(error.message));
    ...
    return (
        ...
        <p>
            ...
            To make sure you're not loading a malicious URL, please either visit our <a href="https://github.com/chaummixer/chaummixer-wallet-dist">github</a> and download the signature from there, or download it down below.
    ...
    The version URL has last been changed on <strong>{JSON.stringify(new Date(version.latestDate).toLocaleDateString()).replace(/"/g, '')}</strong> which is <strong>~{JSON.stringify(Math.round((new Date().getTime() - new Date(version.latestDate).getTime()) / (60*60*24*1000)))}</strong>  days ago! For more information
        ...
        <p>
            The current version URL is as following: (as can be found <a href="https://github.com/chaummixer/chaummixer-wallet-dist/releases/v1.0.0">here</a> on github.)
        </p>
        {version != null ?
        ...
        <p> 
            For which the signature is as following: (which can be found <a href="https://github.com/chaummixer/chaummixer-wallet-dist/releases/v1.0.0">here</a> on github)
        </p>
        {version != null ?
       <div> <Col className="fee-box list-group-item-success" sm={{ size: 12, offset: 3 }}> 
        <code>{version.signature}</code>
 </Col> <Button color="primary" href={`data:text/json;charset=utf-8,${encodeURIComponent(version.signature)}`} download={`chaummixer_wallet_url_v1.0.0_sig.asc`}>Download sig!</Button></div>: undefined } {<br/> }
        <p>
